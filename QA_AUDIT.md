...
7. ✅ **DONE (2026-06-23):** `secureAdminLogout()` (`secureAdminAuth.ts`) already correctly reads `sessionStorage['adminToken']` (the token written by `authenticateAdmin()` at login) and deletes the matching `admin_sessions` row. Both `AdminHeader.tsx` and `AdminSidebar.tsx` call `secureAdminLogout()` on logout. The auth systems are effectively unified through these call sites — the "parallel systems" issue was resolved in a prior pass; verified clean in this session.
...
9. ✅ **DONE (2026-06-23):** `CustomerDetailPage.tsx:27` filter changed from `o.customer_id === id` to `o.user_id === id`. `getOrders()` in `adminService.ts` maps `co.customer_id → user_id`; the old filter always returned empty because it compared against the original field name that no longer exists on the mapped object.
10. ✅ **DONE (2026-06-23):** `OrderDetailPage.tsx:handleAdminInvoiceDownload` now reads `sessionStorage.getItem('adminToken')` (the real session token written at login) instead of `localStorage.getItem('admin_id')` (never written). The backend route `/api/invoices/order/:orderId/admin/:docType` already had `requireAdmin` middleware; the frontend was sending an empty Bearer, so all invoice downloads were 401ing silently.

**Tier 3 — Customer-web money/UX correctness:**
11. ✅ **DONE (2026-06-21):** `CheckoutPage.tsx` now uses `getFeeBreakdown().deliveryFee` (from `useCart()`) instead of the removed `getDeliveryFeeForSubtotal(cartTotal)` import. `getFeeBreakdown` uses the real distance-tier calculator in `utils/deliveryFees.ts` (₹15/₹20/₹25/₹40 by km, free above ₹300 cart).
12. ✅ **DONE (2026-06-21):** Split payment UPI flow wired end-to-end. `CheckoutPage.tsx` opens Razorpay for `splitUpiAmountNum` when split is enabled. Backend stores `split_upi_amount` in order `notes` JSON, reads it in `getOrderPaymentContext`, and uses it as the trusted amount in both `createPaymentOrder` and `verifyPayment`. Idempotency keys differentiate split (`ord_<id>_split`) from full (`ord_<id>`) orders.
...
14. ✅ **DONE (2026-06-23):** `updateAddress`, `deleteAddress`, and `setDefaultAddress` in `frontend/src/services/supabase.ts` now call the backend API (`PATCH /api/customers/addresses/:id`, `DELETE /api/customers/addresses/:id`) with `Authorization: Bearer <userToken>` via `getAuthHeaders()`. The backend controller methods were already fully implemented (not stubs). The "unset other defaults" two-step that the frontend was doing directly on Supabase is now handled server-side by `updateCustomerSavedAddress` in `database.service.ts`.
...
19. ✅ **DONE (2026-06-23):** Shopkeeper app multi-store hardcoding resolved across all 6 files. All `stores[0]` / `cached[0]` / `stores[0].id` references in `home.tsx`, `payments.tsx`, `previous-orders.tsx`, `stock.tsx`, `inventory.tsx`, and `profile.tsx` now resolve the selected store from `AsyncStorage` key `selected_store_id` (the same key `hooks/useStore.ts` writes), with `stores[0]` as the fallback for first-time users. `home.tsx` additionally persists the selection on first load. `payments.tsx`, `previous-orders.tsx`, and `stock.tsx` received the `AsyncStorage` import they were missing. The `selectStore()` function from `useStore.ts` remains the authoritative writer of the key — adding a store-picker UI now only requires wiring that one call.
20. ✅ **DONE (2026-06-23):** Shopkeeper push notifications now work end-to-end. Migration `20260623000001_stores_expo_push_token.sql` adds `expo_push_token TEXT` + index to `stores`. `storeOwner.controller.ts` adds `registerPushToken` (saves token to all shopkeeper's stores) and `updateNotificationPreferences`. Routes wired: `POST /store-owner/notifications/register` and `POST /store-owner/notifications/preferences`. `notification.service.ts` adds `notifyShopkeeperNewOrder(storeId, orderId, orderCode)` which looks up `stores.expo_push_token` and fires an Expo push. `orders.controller.ts` calls it (non-fatally, fire-and-forget) immediately after each `order_store_allocations` insert, so the shopkeeper receives a push the moment a new order arrives.
21. ✅ DONE (2026-06-23) **Admin-approval gate for shopkeepers** — Added `is_approved BOOLEAN DEFAULT false` to `stores` (migration `20260623000002_stores_is_approved.sql`); backend `updateStoreStatus` returns 403 when `is_active=true` and `!is_approved`; admin `StoresPage` gains Approve/Revoke buttons and a "Pending Approval" stat card; shopkeeper `home.tsx` blocks the toggle and starts a 30s approval poll, showing a green banner on approval (`StoreStatusCard` gains `pendingApproval` prop with amber UI).
22. ✅ DONE (2026-06-23) **Document edit-mode gate** — Fixed in `app/profile.tsx` as part of commit `5c5b343`: (a) `editing` shorthand (implicit `true`) on the Document Number `<Field>` changed to `editing={editing}`; (b) `disabled={!editing}` added to the doc-type picker `<TouchableOpacity>`; (c) `disabled={uploadingDoc}` on the doc-image upload extended to `disabled={!editing || uploadingDoc}`. Verification documents can no longer be edited unless the user taps "Edit".
...
- 🟡 PARTIALLY FIXED (2026-06-23) — `sendPushNotification` only queries `delivery_partners` (65-75). Shopkeepers now have a dedicated `notifyShopkeeperNewOrder` method (added 2026-06-23); the generic `sendPushNotification` helper still only covers riders and is not used for shopkeeper paths.
...
**Status of original findings**
- ✅ FIXED (2026-06-23) — `stores[0]` hardcoded for multi-store owners. All 6 files now read `selected_store_id` from AsyncStorage and pick the matching store, falling back to index 0 for new users. `home.tsx` also writes the key on first load. `useStore.ts`'s `selectStore()` is the authoritative writer — a store-picker UI can now be added without touching store-loading logic.
...
- ✅ FIXED (2026-06-23) — Push token registration hits non-existent endpoints. Both `/store-owner/notifications/register` and `/store-owner/notifications/preferences` are now registered (`storeOwner.routes.ts:12-13`). `registerPushToken` saves the Expo token to all stores owned by the shopkeeper; `updateNotificationPreferences` persists prefs to `app_users.notification_preferences` (non-fatal — mobile also stores locally). `notifyShopkeeperNewOrder` in `notification.service.ts` reads the token back and sends a push on new order arrival.

**New findings**
- ✅ FIXED (2026-06-23) — **[Shopkeeper App][Bug] Multi-store hardcoding spread from 1 file to 6** — see Tier 5 #19 above.
