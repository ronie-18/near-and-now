...
2. **Several routers still have zero authentication**: `tracking.routes.ts`, `notifications.routes.ts`, `coupons.routes.ts`, `delivery.routes.ts`, invoice generate/admin routes, plus the checkout endpoints `orders.routes.ts:/place` and `/create` (which trust `user_id`/`customer_id` from the request body — closing this fully requires the checkout flow to send the session token too). Anyone who knows/guesses a UUID can still write tracking updates, coupons, and delivery partner records, or place orders impersonating another customer_id.
3. **Razorpay webhook HMAC verification is still broken** (`backend/src/services/payment.service.ts:393` stringifies an already-parsed body instead of using the raw bytes; line 389 falls back to `RAZORPAY_KEY_SECRET` when the webhook secret is unset, and that secret isn't even in `.env.example`). All webhook-driven payment status updates are unreliable/forgeable in production — despite commit `a174547`'s message claiming this was fixed.
4. **`order_store_allocations` insert failures are silently swallowed** (`orders.controller.ts:97-107`, `console.error('...non-fatal...')`) — most plausible cause of orders that never reach a shopkeeper.
5. **`customer_orders.total_amount`/`delivery_fee`/`subtotal_amount` hardcoded to 0** in the legacy `createCustomerOrder` path (`database.service.ts:282-314`), and `orders.controller.ts:74` separately hardcodes `delivery_fee: 20` on the per-store row — these never reconcile. Combined with `payment.service.ts:123-125` throwing on `total_amount <= 0`, any payment attempt on an order from this path hard-fails.
6. **`storeOwner.controller.ts` has no real authorization anywhere** — `getStores` reads `userId` from a query param, `updateStoreStatus`/`updateProductQuantity`/`updateStore` only check that *a* Bearer header is present, never whose it is or whether it owns the target store/product. Any caller can read, mutate, or impersonate any store owner.
