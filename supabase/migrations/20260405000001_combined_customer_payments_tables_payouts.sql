-- =============================================================================
-- Part 2 of 2 — customer_orders, customer_payments, payouts, ledger, order_summary
--
-- Run after 20260405000000_combined_customer_payments_enums_payouts.sql (committed).
--
-- Prerequisites: public.customer_orders, app_users, stores, store_orders,
-- order_items; function public.update_updated_at_column(); enum types from Part 1.
--
...
-- order_summary (and any view that references these columns) blocks ALTER COLUMN TYPE.
drop view if exists public.order_summary cascade;

-- store_orders must expose customer_order_id before Part 3 (payments join), Part 6 (store_payouts backfill), and Part 7 (view).
do $$
begin
  if not exists (
  ...
  if exists (
    ...
    where table_schema = 'public' and table_name = 'store_orders' and column_name = 'customer_order_id'
  ) then
    return;
  end if;
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'store_orders' and column_name = 'order_id'
  ) then
    alter table public.store_orders rename column order_id to customer_order_id;
  elsif exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'store_orders' and column_name = 'customer_orders_id'
  ) then
    alter table public.store_orders rename column customer_orders_id to customer_order_id;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'store_orders' and column_name = 'customer_order_id'
    ...
    raise exception
      using message = 'public.store_orders has no customer_order_id (rename order_id / customer_orders_id or add column referencing customer_orders.id).';
  end if;
end $$;
...
begin
  ...
  if has_payments and not has_cp then
    ...
    -- Legacy FK column name (expected: customer_order_id)
    if not exists (
      select 1 from information_schema.columns
      where table_schema = 'public' and table_name = 'payments' and column_name = 'customer_order_id'
    ) then
      if exists (
        select 1 from information_schema.columns
        where table_schema = 'public' and table_name = 'payments' and column_name = 'order_id'
      ) then
        alter table public.payments rename column order_id to customer_order_id;
      elsif exists (
        select 1 from information_schema.columns
        where table_schema = 'public' and table_name = 'payments' and column_name = 'customer_orders_id'
      ) then
        alter table public.payments rename column customer_orders_id to customer_order_id;
      end if;
    end if;
    ...
    if not exists (
      ...
      raise exception
        using message = 'public.payments has no customer_order_id (add it or rename order_id / customer_orders_id to match customer_orders.id).';
    end if;
    ...
    exception
      when undefined_object then
        begin
          alter table public.customer_payments rename constraint payments_order_id_key to customer_payments_customer_order_id_key;
        exception
    ...
    exception
      when undefined_object then
        begin
          alter table public.customer_payments rename constraint payments_order_id_fkey to customer_payments_customer_order_id_fkey;
        exception
          when undefined_object then null;
        end;
    end;
    ...
    exception
      when undefined_object then
        begin
          alter index public.idx_payments_order_id rename to idx_customer_payments_customer_order_id;
        exception
          when undefined_object then null;
        end;
    end;
...
-- Pre-existing customer_payments may use order_id instead of customer_order_id
do $$
begin
  if exists (
    select 1 from information_schema.tables
    ...
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'customer_payments' and column_name = 'customer_order_id'
  ) then
    if exists (
      select 1 from information_schema.columns
      where table_schema = 'public' and table_name = 'customer_payments' and column_name = 'order_id'
    ) then
      alter table public.customer_payments rename column order_id to customer_order_id;
    elsif exists (
      select 1 from information_schema.columns
      where table_schema = 'public' and table_name = 'customer_payments' and column_name = 'customer_orders_id'
    ) then
      alter table public.customer_payments rename column customer_orders_id to customer_order_id;
    end if;
  end if;
end $$;
...
begin
  ...
  if exists (
    ...
    execute 'create index if not exists idx_customer_payments_customer_order_id on public.customer_payments using btree (customer_order_id) tablespace pg_default';
  end if;

  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'customer_payments' and column_name = 'customer_id'
  ) then
    execute 'create index if not exists idx_customer_payments_customer_id on public.customer_payments using btree (customer_id) tablespace pg_default';
  end if;

  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'customer_payments' and column_name = 'razorpay_payment_id'
  ) then
    execute 'create index if not exists idx_customer_payments_razorpay_payment_id on public.customer_payments using btree (razorpay_payment_id) where razorpay_payment_id is not null';
  end if;

  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'customer_payments' and column_name = 'updated_at'
  ) then
    execute 'drop trigger if exists update_customer_payments_updated_at on public.customer_payments';
    execute 'create trigger update_customer_payments_updated_at before update on public.customer_payments for each row execute function public.update_updated_at_column ()';
  end if;

  execute 'comment on table public.customer_payments is ''Customer payment per order; links customer_orders + app_users.''';
end $$;
...
-- CREATE TABLE IF NOT EXISTS skips DDL when store_payouts already exists; older tables may lack customer_order_id.
do $$
begin
  if not exists (
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'store_payouts'
  ) then
    return;
  end if;
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'store_payouts' and column_name = 'customer_order_id'
  ) then
    return;
  end if;
  alter table public.store_payouts add column customer_order_id uuid;
  update public.store_payouts sp
  set customer_order_id = so.customer_order_id
  from public.store_orders so
  where sp.store_order_id is not null
    and sp.store_order_id = so.id
    and sp.customer_order_id is null;
  begin
    alter table public.store_payouts alter column customer_order_id set not null;
  exception
    when others then
      raise notice 'store_payouts.customer_order_id left nullable (backfill could not fill all rows)';
  end;
  begin
    alter table public.store_payouts
      add constraint store_payouts_customer_order_id_fkey
      foreign key (customer_order_id) references public.customer_orders (id) on delete cascade;
  exception
    when duplicate_object then null;
  end;
...
begin
  if exists (
    ...
    execute 'create index if not exists idx_store_payouts_customer_order_id on public.store_payouts using btree (customer_order_id) tablespace pg_default';
  end if;
end $$;
