...
# Parse the XML file
tree = ET.parse('temp_dals/xl/worksheets/sheet1.xml')
root = tree.getroot()
...
VALUES ('{name}', {price}, {original_price}, 'staples', '{size}', '{image_url}', true, 4.5);"""
    
    sql_statements.append(sql)

# Write to SQL file with proper newlines
with open('insert_dals_pulses.sql', 'w', encoding='utf-8') as f:
    f.write("-- Insert Dals & Pulses products into staples category\n")
    f.write(f"-- Total products: {len(products)}\n")
    f.write("-- Generated automatically\n")
    f.write("-- Run this script in Supabase SQL Editor\n")
    f.write("-- NOTE: This assumes 'staples' category already exists in the database\n\n")
    
    f.write("-- STEP 1: Ensure products table auto-generates IDs\n")
    f.write("ALTER TABLE products ALTER COLUMN id SET DEFAULT gen_random_uuid()::text;\n\n")
    
    f.write("-- STEP 2: Insert all products into existing 'staples' category\n")
    f.write("-- If the category name is different (e.g., 'Staples' with capital S),\n")
    f.write("-- you may need to update the category name in the INSERT statements below\n\n")
    for sql in sql_statements:
        f.write(sql + '\n\n')

print(f"\n✅ SQL file generated: insert_dals_pulses.sql")
